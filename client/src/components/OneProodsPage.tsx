...
import { Button, CloseButton, Form, Pagination } from 'react-bootstrap';
...
export const OneProodsPage = (props: any) => {
  ...
  const raits: any[] = [1, 2, 3, 4, 5];
  const item: any = useSelector((state: any) => state.oneProd.oneProds);
  const arrayRaiting: string[] = String(item.raiting).split(',');
  const averageRaiting: string = (
    arrayRaiting.reduce((total: any, elem: any) => total + Number(elem), 0) / arrayRaiting.length
  ).toFixed(1);
    ...
    <div>
      {item.length >= 0 ? (
        ...
        <div className={s.main}>
          <div className={s.main_div_oneProd}>
            <div className={s.main_div_wrapper}>
              <div className={s.image_div} style={{ backgroundImage: `url(${item.url})` }}>
                {' '}
              </div>
              ...
              <div className={s.raiting}>
                <div className={s.star}>Рейтинг товара: ★</div>
                {averageRaiting}{' '}
              </div>
              <div className={s.setRaiting}>
                {raits.map((elem: any, index: any) => (
                  <div
                    key={index}
                    className={s.stars}
                    onClick={() =>
                      newAuth !== null
                        ? setRaiting(elem)
                        : alert('Ставить оценки могут только зарегистрированные пользователи')
                    }
                  >
                    ★<div className={s.setOneRaiting}>{elem}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className={s.review}>
                    ...
                    <div key={index} className={s.oneMessage}>
                      {
                        <>
                          <div
                            style={{
                              backgroundImage: elem?.imgPerson
                                ? `url(${elem.imgPerson})`
                                : `url(${image})`,
                            }}
                            className={s.imgPersonMessage}
                          ></div>
                          <div className={s.inform}>
                            <div className={s.nickname}>
                              {newAuth !== null ? (
                                <h5>{elem.namePerson}</h5>
                              ) : (
                                <>{elem.namePerson}</>
                              )}
                            </div>
                          ...
                          {role.role == 'ADMIN' ? (
                            <CloseButton
                              onClick={() => DeleteMessage(elem.id)}
                              variant="white"
                              className={s.AdminRemoveMessage}
                            />
                          ) : (
                            <></>
                          )}
            ...
            <div className={s.FormSendReview}>
              <Form onSubmit={handleSubmit(sendMessages)} className={s.FormSendReviewCrap}>
                ...
                />
                <Button className={s.buttonSubmit} type="submit" variant="outline-light">
                  Отправить
                </Button>
              </Form>
            </div>
            <div className={s.messageError}>
              <div className={s.erros}>{errors?.text_message?.message}</div>
            </div>
            <div className={s.pagination}>
              <Pagination className={s.paginationArray} size="sm">
                {items}
              </Pagination>
            </div>
          </div>
