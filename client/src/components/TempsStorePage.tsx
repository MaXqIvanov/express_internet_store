...
export const TempsStorePage = (props: any) => {
  ...
  useEffect(() => {
    setChecks(
      localStorage.getItem(props.elem.name) != null
        ? JSON.parse(String(localStorage.getItem(props.elem.name)))
        : ''
    );
  }, [nullInfo]);
  ...
  return (
    <div className="tempStorePage_div">
      <div title="Узнать подробнее" className="block_1_store_page">
        <div
          onClick={() => {
          ...
          className="img_all_storePage"
          style={{ backgroundImage: `url(${props.elem.url})` }}
        ></div>
      </div>
      {Number(props.elem.id) != Number(checks.id) ? (
        <div
      ...
      <div
        title="Узнать подробнее о товаре"
        className="div_storePage_explain"
        onClick={() => {
          dispatch(changeStateProods({ id: props.elem.id, elem: props.elem }));
          dispatch(changeRaiting({ id: props.elem.id, raiting: props.elem.raiting }));
          history('/' + props.elem.id);
