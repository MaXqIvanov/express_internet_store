    ...
    <div id="open-chat" className={s.main}>
          ...
          <div className={s.wrapper_top_left_centr}>
            <div className={s.wrapper_top_left_centr_info}>
              ...
              <div className={s.phone}>
                <div className={s.image_phone}></div> <div>89531234561</div>
              </div>
              <div className={s.laptope}>
                <div className={s.image_laptope}></div> <div>web-liter@email.ru</div>
              </div>
            </div>
          </div>
        </div>
        <div className={s.wrapper_top_right}>
          <div className={s.wrapper_top_right_centr}>
            <div className={s.wrapper_top_right_centr_info}>
              <h5>• Соц сети: </h5>
              <div className={s.vk}>
                <div title="перейти в VK" className={s.image_vk}></div>{' '}
                <div title="перейти в telegram" className={s.image_telegram}></div>{' '}
                <div title="перейти в whatsapp" className={s.image_whatsapp}></div>
      ...
      <YMaps query={{ apikey: process.env.REACT_APP_YANDEX_MAPS, load: 'package.full' }}>
        <div className={s.wrapper_bottom}>
            ...
            <div className={s.pick_up_points}>
              <div className={s.points_wrapper}>
                {mock.map((elem: any, index: any) => (
                  <div onClick={() => setPonter(elem.geoData)} key={index} className={s.points}>
                    <div>{elem.title}</div>
          ...
          <div className={s.Map_load}>
            <Spinner className={s.SpinnerContacts} animation={'border'}></Spinner>
          </div>
          <Map
            className={s.Map}
            style={{ color: 'white' }}
            state={{ center: [...pointer], zoom: 11, behaviors: ['default', 'scrollZoom'] }}
            defaultState={{ center: [...pointer], zoom: 11, behaviors: ['default', 'scrollZoom'] }}
          >
            <Clusterer
              options={{
