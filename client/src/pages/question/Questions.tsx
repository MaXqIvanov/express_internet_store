...
export const Questions = (props: any) => {
  ...
  const onsubmitMessageError = async (data: any) => {
    const message =
      'name: ' + String(data.name_message) + ', ' + 'massage: ' + String(data.errors_message);
    ...
    <div className={s.main}>
      <div className={s.wrapper}>
        ...
        <Form onSubmit={handleSubmit(onsubmitMessageError)} className={s.Form}>
          ...
          />
          <div className={s.nickName_Button}>
            <Form.Control
              {...register('name_message', {
                required: 'Поле обязательно для заполнения',
                minLength: {
                  value: 3,
                  message: 'минимальная длина 3 символа',
              ...
              className={s.name_message}
              autoComplete="off"
            />
            <Button disabled={stateButtons} type="submit" className={s.ButtonSub}>
              отправить
            </Button>
          </div>
          <div className={s.erros_Div}>
            {errors?.errors_message ? (
              <p style={{ color: 'red' }}>{errors?.errors_message?.message}</p>
            ) : errors?.name_message ? (
              <p style={{ color: 'red' }}>{errors?.name_message?.message}</p>
            ) : (
              <></>
            )}
          </div>
