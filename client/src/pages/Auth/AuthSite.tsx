      ...
      {registerSite ? (
        <div className={s.authSite}>
          <div
            title="назад"
            className={s.navHome}
            onClick={() => props.object.setAuthSite(!props.object.authSite)}
          ></div>
          <div
            onClick={() => setRegisterSite(!registerSite)}
            title="зарегистрироваться на сайте"
            className={s.div_auth2}
          >
            нет аккаунта?
          </div>
          <div
            className={s.img_hidden}
            onClick={() => {
              const g = visiblePass;
            ...
            <Form onSubmit={handleSubmit(onAuth)} className={s.formGroup}>
              <Form.Label>
                ...
                <div>
                  Почта :{' '}
                  {errors?.LoginEmail && (
                    <span style={{ color: 'red' }}>{errors?.LoginEmail?.message}</span>
                  )}
                </div>
              </Form.Label>
              <Form.Label>
                ...
                {errors?.LoginPassword && (
                  <span className={s.errors} style={{ color: 'red' }}>
                    {errors?.LoginPassword?.message}
                  </span>
                )}
              </Form.Label>
              <Button type="submit" className={s.button_login} variant="outline-light">
                <div>Войти</div>
              </Button>
            </Form>
          </div>
        ...
        <div className={s.authSite}>
          ...
          ></div>
          <Form onSubmit={handleSubmit(onRegistration)} className={s.formGroup}>
            <Form.Label>
              <Form.Control
                className={s.Form}
              ...
              <div>
                ...
                {errors?.RegisterEmail && (
                  <span style={{ color: 'red' }}>{errors?.RegisterEmail?.message}</span>
                )}
              </div>
            </Form.Label>
            <Form.Label>
              <Form.Control
                ...
                {...register('RegisterPassword', {
                  required: 'не заполнено',
                  pattern: { value: /[A-Za-z1-9а-яА-Я]{5}/, message: 'слабая надёжность' },
                })}
                type={visiblePass}
              ></Form.Control>
              Пароль:{' '}
