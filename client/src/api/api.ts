...
export const apiGetGoods = (
  ...
  axios
    .get(
      `http://127.0.0.1:5000/api/goods/${typeGoods}` +
        `?page=${page}` +
        `&sort=${sortPrice}` +
        `&name=${name}`
    )
...
export const apiSetRaiting = (
  ...
  axios
    .post(`http://localhost:5000/api/goods/${item.id}`, {
      raiting: elem,
      auth: auth.authAuth,
    ...
    .then((response: any) => {
      response.data
        ? dispatch(changeRaiting({ id: item.id, raiting: arrayRaiting.join(',') }))
...
export const AdminGetProods = (auth: any, setData: any, dispatch: any) => {
  axios
    .post('http://localhost:5000/api/admin/1', {
      email: auth.emailAuth,
    })
    .then((response: any) => {
      const obj = { role: response.data.role, email: response.data.email };
...
export const AdminAddProods = (data: any, auth: any, password: any) => {
  axios
    .post('http://127.0.0.1:5000/api/goods', {
...
export const AdminRemoveProods = (password: any, auth: any, data: any) => {
  axios
    .post(`http://localhost:5000/api/goods/delete/${data.id}`, {
      password: password,
      email: auth.emailAuth,
    })
...
export const AdminGetAllProods = (type: any, setAllProods: any) => {
  axios
    .get(`http://localhost:5000/api/goods/${type}?limit=100`)
    .then((data: any) => setAllProods(data.data.rows));
};
// ____________________________________________________________________________________________
...
export const RegisterUsersForms = (data: any, setIsRegistr: any, isRegistr: any, token: any) => {
  axios
    .post('http://localhost:5000/api/users/regist', {
...
export const APIOrders = (rUp: any, price: any, reradeName: any, reradeNumber: any) => {
  axios.post('http://localhost:5000/api/orders', {
    name: `${rUp}`,
    price: Number(price),
    namePerson: reradeName,
    telPerson: String(reradeNumber),
  });
};
// _____________________________________________________________________________________________
export const APIGetMessages = (item: any, setMessages: any, setCountMessages: any, active: any) => {
  axios
    .get(`http://localhost:5000/api/messages/${item.id}?page=${active}&limit=10`)
    .then((data: any) => {
...
export const APISendMessages = (
  ...
  axios
    .post(`http://localhost:5000/api/messages/${item.id}`, {
      idProods: item.id,
      messages: data.text_message,
      imgPerson: newAuth.imageAuth,
      namePerson: newAuth.nameAuth,
      email: newAuth.emailAuth,
    })
    .then((data: any) => {
...
export const APIDeleteMessage = (
  ...
  axios
    .post(`http://localhost:5000/api/messages/delete/${data}`, {
      email: email,
      password: pass,
    })
...
export const APITelegramMessage = (
  token: any,
  message: string,
  setStateButtons: any,
  ...
  axios
    .post(`http://localhost:5000/api/messages/send/telegram`, {
      token: token,
